...
void PresetCommand::Run(uint32_t parBits, uint16_t presetNumber)
{
	uint8_t selectedParIndex = 0;
	for (uint8_t parNumber = 0; parNumber < NR_OF_PARS; parNumber++)
	{
		if ((parBits & (0x8000 >> parNumber)) > 0)
		{
			uint8_t nrOfPars = BitsUtils::GetNrOfHighBits(parBits);
...
void PresetCommand::SetFixturePreset(uint16_t presetNumber, uint8_t parNumber, uint8_t selectedParIndex, uint8_t nrOfPars)
{
	Par& par = LightSetup.GetPar(parNumber);
	uint16_t nrOfSteps = 1;
	...
	case 30:
		...
		par.InitializeProgram(10, 2, 0, 0, 0);
		break;

	case 31:
		// Switch between alternate and default color
		par.InitializeProgram(10, 2, 1, 0, 0);
		break;
	...
	case 41:
		// Chase right -> left
		par.InitializeProgram(10, nrOfPars, 0, nrOfPars - selectedParIndex - 1, nrOfPars - selectedParIndex - 1);
		break;

	case 42:
		// Chase left -> right -> left
		nrOfSteps = (nrOfPars - 1) * 2;
		par.InitializeProgram(10, nrOfSteps, 0, selectedParIndex, (nrOfSteps - selectedParIndex) % nrOfSteps);
		break;

	case 43:
		// Chase right -> left -> right
		par.InitializeProgram(10, (nrOfPars - 1) * 2, 0, nrOfPars - 1 - selectedParIndex, nrOfPars - 1 + selectedParIndex);
		break;
	...
	case 52:
		...
		par.InitializeProgram(20, MAX_PAR_INTENSITIES * 2 - 2, 0);
		break;

	case 53:
		// Fade alternate color -> default color -> alternate color
		// Start halfway.
		par.InitializeProgram(20, MAX_PAR_INTENSITIES * 2 - 2, MAX_PAR_INTENSITIES);
		break;
